...
class ImageEmbedder(nn.Module):
    ...
    def forward(self, img):
        x = self.efficient_net.extract_features_at_block(
            img, self.efficient_block)
        '''
        x_scaled = []
...
class CrossEfficientViT(nn.Module):
    def __init__(
        ...
        self.lg_image_embedder = ImageEmbedder(
            dim=lg_dim, image_size=image_size, patch_size=lg_patch_size, dropout=emb_dropout, efficient_block=1, channels=lg_channels)
